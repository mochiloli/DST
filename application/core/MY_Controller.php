...
class MY_Controller extends MX_Controller {
    ...
    protected function render($view_file, $layout = 'default') {
        ...
        $seo = $this->mo_main_info->get_all();

        if (empty($this->mViewData['title'])) {
            $this->mViewData['title'] = $seo[0]->title_tag;
        }
        if (empty($this->mViewData['description'])) {
            $this->mViewData['description'] = $seo[0]->meta_description;
        }
        if (empty($this->mViewData['keyword'])) {
            $this->mViewData['keyword'] = $seo[0]->meta_keyword;
        }
        if (empty($this->mViewData['og_url'])) {
            $this->mViewData['og_url'] = $seo[0]->og_url;
        }
        if (empty($this->mViewData['og_type'])) {
            $this->mViewData['og_type'] = $seo[0]->og_type;
        }
        if (empty($this->mViewData['og_title'])) {
            $this->mViewData['og_title'] = $seo[0]->og_title;
        }
        if (empty($this->mViewData['og_description'])) {
            $this->mViewData['og_description'] = $seo[0]->og_description;
        }
        if (empty($this->mViewData['og_image'])) {
            $this->mViewData['og_image'] = base_url('assets/uploads/main_info/').$seo[0]->og_image;
        }
